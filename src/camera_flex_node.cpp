...
CameraFlexNode::CameraFlexNode(const rclcpp::NodeOptions &options)
  ...
  try {
    ...
    RCLCPP_FATAL(this->get_logger(), "Failed to load config file: %s",
                 e.what());
    rclcpp::shutdown();
    return;
  }
  ...
  if (!cap_.isOpened()) {
    ...
    RCLCPP_WARN(this->get_logger(),
                "Failed to open camera with V4L2, trying default backend");
    cap_.open(device);
    if (!cap_.isOpened()) {
      RCLCPP_FATAL(this->get_logger(), "Failed to open camera device: %d",
                   device);
      rclcpp::shutdown();
      return;
    }
  ...
  RCLCPP_INFO(this->get_logger(), "Camera FOURCC format: %s", fourcc_str);
  ...
  for (std::size_t i = 0; i < pipelines_node.size(); ++i) {
    ...
    RCLCPP_INFO(this->get_logger(), "Added pipeline: %s -> %s",
                pipelines_.back().name.c_str(),
                pipelines_.back().topic.c_str());
  }
...
void CameraFlexNode::initialize() {
  ...
  RCLCPP_INFO(this->get_logger(), "Initializing %zu pipelines",
              pipelines_.size());
  ...
  RCLCPP_INFO(this->get_logger(), "Timer created with period: %d ms",
              1000 / fps_);
...
void CameraFlexNode::captureLoop() {
  ...
  if (!format_logged) {
    ...
    switch (frame.type()) {
    case CV_8UC1:
      type_str = "CV_8UC1 (GRAY)";
      break;
    case CV_8UC3:
      type_str = "CV_8UC3 (BGR)";
      break;
    case CV_8UC4:
      type_str = "CV_8UC4 (BGRA)";
      break;
    default:
      type_str = "Unknown type: " + std::to_string(frame.type());
      break;
    }
  ...
  if (frame.channels() == 3) {
    ...
    RCLCPP_WARN_ONCE(this->get_logger(),
                     "Unsupported channel count: %d, using frame as-is",
                     frame.channels());
    bgr_frame = frame.clone();
  }
  ...
  // 10フレームごとにログ出力
  if (++frame_count % 30 == 0) {
    RCLCPP_INFO(this->get_logger(), "Processed frame %d: %dx%d", frame_count,
  ...
  for (auto &pipe : pipelines_) {
    try {
      ...
      if (processed.channels() == 3) {
        ...
        RCLCPP_WARN_ONCE(this->get_logger(),
                         "Unexpected channel count %d, using BGR8 encoding",
                         processed.channels());
        encoding = sensor_msgs::image_encodings::BGR8;
      }
      ...
      if (frame_count <= 3) {
        RCLCPP_INFO(this->get_logger(),
                    ...
                    pipe.topic.c_str(), encoding.c_str(), processed.cols,
                    processed.rows);
      }
    } catch (const std::exception &e) {
      RCLCPP_ERROR_ONCE(this->get_logger(), "Failed to publish on topic %s: %s",
                        pipe.topic.c_str(), e.what());
    }
  }
}

cv::Mat CameraFlexNode::applyOperations(const cv::Mat &frame,
  ...
  for (const auto &op : ops) {
    if (op.type == "crop") {
      ...
      if (w > 0 && h > 0) {
        ...
        RCLCPP_DEBUG(this->get_logger(), "Applied crop: (%d,%d) %dx%d", x, y, w,
                     h);
      } else {
        RCLCPP_WARN(this->get_logger(),
                    "Invalid crop region, skipping crop operation");
      }
      ...
      if (w > 0 && h > 0) {
        ...
        RCLCPP_WARN(this->get_logger(),
                    "Invalid resize dimensions, skipping resize operation");
      }
      ...
      if (enc == "HSV") {
        ...
        if (result.channels() == 3) {
          ...
          RCLCPP_DEBUG(this->get_logger(),
                       "Image is already grayscale, skipping conversion");
        }

      } else {
        RCLCPP_WARN_ONCE(this->get_logger(), "Unsupported color encoding: %s",
                         enc.c_str());
      }
    } else {
      RCLCPP_WARN_ONCE(this->get_logger(), "Unknown operation type: %s",
                       op.type.c_str());
    }
  }
